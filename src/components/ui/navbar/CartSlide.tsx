...
const CartSlide = () => {
  ...
  >
    <button onClick={() => setIsCartOpen(!isCartOpen)} className="text-gray-900 dark:text-gray-200 hover:text-primary relative">
      <span className="text-2xl"><BsCart3 className="w-6 h-6" /></span>
      {cartItems.length > 0 && (
    ...
    {isCartOpen && (
      <div className="absolute right-0 mt-2 w-80 bg-gray-100 text-gray-900 dark:bg-gray-900 dark:text-gray-200 border border-gray-200 dark:border-gray-800 rounded-lg shadow-lg z-50">
        <div className="p-4">
            ...
            <div><h3 className="font-poppins font-semibold text-lg ">Your Cart</h3>
            <p className="text-xs">Review your items and proceed to checkout.</p></div>
            <button onClick={() => setIsCartOpen(false)} className='text-2xl'>X</button>
            </div>
         
          {cartItems.length === 0 ? (
            <p className="text-gray-900 dark:text-gray-200">Your cart is empty.</p>
          ) : (
            <div>
              ...
              <div className="mt-4 pt-4 border-t border-gray-200">
                ...
                <button onClick={handleCheckout} className="w-full mt-2 bg-primary text-white px-4 py-2 rounded hover:bg-primary-dark">
                  Place Order
                </button>
              </div>
            </div>
